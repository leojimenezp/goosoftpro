...
router.post('/repor/busqueda', isLoggedIn, async (req, res) => {
    ...
    const ejecutado = await pool.query(`
        SELECT
            ttcgp.tipo,
            SUM(IF( ttcgp.id_moneda = '1', ttcgp.total * tp.trm ,(ttcgp.cant * ttcgp.valor))) AS total
        FROM tb_ticket tt, tb_ticket_copia_gatos_planeacion ttcgp, tb_monedas tm ,tb_planeacion tp
        WHERE tt.id_servicio = '${id_planeacion}'
            AND tt.id = ttcgp.id_ticket
            AND ttcgp.id_moneda = tm.id_moneda 
            AND tt.id_servicio = tp.id_planeacion
        GROUP BY tipo
        ORDER BY tipo
    `);

    
    const costo_cotizaciontbr = await pool.query(`
        SELECT
            ie.id_planeacion ,ie.id_cotizacion_costo ,ie.descripcion, ie.tipo, ie.cantidad,
            u.abreviatura_unidad_medida,ie.precio, m.abreviatura_moneda, SUM(precio * cantidad) AS total
    ...
    const costo_cotizaciontbrc = await pool.query(`
        SELECT
            ie.id_planeacion, ie.id_cotizacion_costo, ie.descripcion, ie.tipo, ie.cantidad,
            u.abreviatura_unidad_medida, ie.precio, m.abreviatura_moneda, SUM(precio * cantidad) AS total
    ...
    let subcontratotbr = [{ suma: parseInt(et1) + parseInt(et2) }];
    let subcontratotbrc = [{ suma: parseInt(t1) + parseInt(t2) }];
