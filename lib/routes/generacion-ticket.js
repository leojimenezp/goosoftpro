...
router.post("/ticket", isLoggedIn, async (req, res) => {
    ...
    let tickets;
    if (ticket > 0) tickets = await pool.query("SELECT tt.id_servicio , tt.id, tpl.titulo, tpl.fecha_estimada, tt.fecha FROM tb_ticket tt INNER JOIN tb_planeacion tpl ON tt.id_servicio = tpl.id_planeacion WHERE tt.id_servicio = ?", [servicio]);else tickets = await pool.query("SELECT tt.id_servicio , tt.id, tpl.titulo, tpl.fecha_estimada, tt.fecha FROM tb_ticket tt INNER JOIN tb_planeacion tpl ON tt.id_servicio = tpl.id_planeacion");
...
router.post("/ticket/get/items", isLoggedIn, async (req, res) => {
    ...
    if (dataItem.length > 0) res.json({ item: dataItem });else res.json({ resp: "No se encontraron datos" });
});
