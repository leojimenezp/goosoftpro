...
router.post('/planeacion/fechas', isLoggedIn, async (req, res) => {
  ...
  console.log(Date.diff(fecha_final, fecha_inicio).years());
  console.log(Date.diff(fecha_final, fecha_inicio).months());
  res.render('planeacion/planeacion', {
    consulta: consulta
  });
});
...
router.get('/planeacion/graficas/:id_planeacion', isLoggedIn, async (req, res) => {
  ...
  const equipo_total_imprevistos = await pool.query(`SELECT SUM(cantidad * costo_unitario) total FROM tb_equipo_item_imprevistos ie, tb_equipos e WHERE ie.id_planeacion = '${id_planeacion}'`); // Movilizacion
...
router.post('/equipo/equipos-herramientas/agregar/:id_planeacion', isLoggedIn, async (req, res) => {
  ...
  console.log(array[array.length - 1]); //await pool.query(`INSERT INTO tb_equipo_item_combustible (id_planeacion,id_item,id_rubro,id_unidad_medida,id_moneda,costo_unitario,medio_pago,fecha_inicio_gasto,fecha_final_gasto,id_equipo_item_equipo_herramienta)
...
router.post('/movilizacion/personal/agregar/:id_planeacion', isLoggedIn, async (req, res) => {
  ...
  console.log(array2[array2.length - 1]); //await pool.query(`INSERT INTO tb_mov_item_combustibles(id_planeacion,id_item,id_rubro,id_unidad_medida,id_moneda,costo_unitario,medio_pago,fecha_inicio_mov,fecha_final_mov,id_mov_item_personal)
  //VALUES('${id_planeacion}','1','${id_rubro}','${id_unidad_medida}','${id_moneda}','${costo_unitario_rubro}','${medio_pago}','${fecha_inicio_mov}','${fecha_final_mov}','${array[array.length - 1]}')`);
...
router.post('/calcularVehiculos_Equipo', isLoggedIn, async (req, res) => {
  ...
  if (cal_m != undefined) {
    ...
    for (var i = 0; i <= verifica.length - 1; i++) {
      await pool.query(`UPDATE tb_equipo_item_equipo_herramienta SET fecha_inicio_gasto = '${f_i_mov}', fecha_final_gasto = '${fe_i_mov}' WHERE id_equipo_item_equipo_herramienta = '${verifica[i]}'`); //await pool.query(`UPDATE tb_equipo_item_combustible SET fecha_inicio_gasto = '${f_i_mov}', fecha_final_gasto = '${f_f_mov}' WHERE id_equipo_item_equipo_herramienta = '${verifica[i]}'`);
    }
  } else if (cal_dm != undefined) {
    const {
      f_i_dm,
